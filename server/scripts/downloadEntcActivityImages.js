...
const defaultsPath = path.resolve(
  __dirname,
  "../../client/src/data/entcDefaults.js",
);
const outputDir = path.resolve(__dirname, "../uploads/images/entc/activities");
const remoteImagePattern =
  /https:\/\/www\.ssgmce\.ac\.in\/images\/extc_faculty\/[^\s"']+/g;
...
const readActivityImageUrls = () => {
  ...
  return [...new Set(fileContent.match(remoteImagePattern) || [])].filter(
    (url) =>
      /\/extc_activity_/i.test(url) && /\.(png|jpe?g|webp|gif)$/i.test(url),
  );
};
...
const main = async () => {
  ...
  console.log(`Found ${urls.length} EnTC activity images.`);
...
main().catch((error) => {
  console.error("Unexpected failure while downloading EnTC activity images.");
  console.error(error);
  process.exit(1);
});
