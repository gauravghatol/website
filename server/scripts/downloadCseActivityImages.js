...
const defaultsPath = path.resolve(__dirname, "../../client/src/data/cseDefaults.js");
const outputDir = path.resolve(__dirname, "../uploads/images/cse/activities");
const remoteImagePattern =
  /https:\/\/www\.ssgmce\.ac\.in\/images\/cse_faculty\/[^\s"']+/g;
...
const main = async () => {
  ...
  console.log(`Found ${urls.length} CSE activity images.`);
...
main().catch((error) => {
  console.error("Unexpected failure while downloading CSE activity images.");
  console.error(error);
  process.exit(1);
});
