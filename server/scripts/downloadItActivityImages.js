...
const defaultsPath = path.resolve(__dirname, "../../client/src/data/itDefaults.js");
const outputDir = path.resolve(__dirname, "../uploads/images/it/activities");
const remoteImagePattern =
  /https:\/\/www\.ssgmce\.ac\.in\/images\/it_faculty\/[^\s"']+/g;
...
const readActivityImageUrls = () => {
  ...
  return [...new Set(fileContent.match(remoteImagePattern) || [])].filter(
    (url) => /\/it_activity/i.test(url) && /\.(png|jpe?g|webp|gif)$/i.test(url),
  );
};
...
const main = async () => {
  ...
  console.log(`Found ${urls.length} IT activity images.`);
...
main().catch((error) => {
  console.error("Unexpected failure while downloading IT activity images.");
  console.error(error);
  process.exit(1);
});
