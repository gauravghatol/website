...
const defaultsPath = path.resolve(
  __dirname,
  "../../client/src/data/mechanicalDefaults.js",
);
const outputDir = path.resolve(
  __dirname,
  "../uploads/images/mechanical/activities",
);
const remoteImagePattern =
  /https:\/\/www\.ssgmce\.ac\.in\/images\/mech_faculty\/[^\s"']+/g;
...
const readActivityImageUrls = () => {
  ...
  return [...new Set(fileContent.match(remoteImagePattern) || [])].filter(
    (url) => /\/mech_activity/i.test(url) && /\.(png|jpe?g|webp|gif)$/i.test(url),
  );
};
...
const main = async () => {
  ...
  console.log(`Found ${urls.length} Mechanical activity images.`);
...
main().catch((error) => {
  console.error(
    "Unexpected failure while downloading Mechanical activity images.",
  );
  console.error(error);
  process.exit(1);
});
