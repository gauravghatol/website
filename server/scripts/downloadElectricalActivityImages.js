...
const defaultsPath = path.resolve(
  __dirname,
  "../../client/src/data/electricalDefaults.js",
);
const outputDir = path.resolve(
  __dirname,
  "../uploads/images/electrical/activities",
);
const remoteImagePattern =
  /https:\/\/www\.ssgmce\.ac\.in\/images\/elect_faculty\/[^\s"']+/g;
...
const readActivityImageUrls = () => {
  ...
  return [...new Set(fileContent.match(remoteImagePattern) || [])].filter(
    (url) =>
      /\/(elect_activity_|electrical_activity_)/i.test(url) &&
...
const main = async () => {
  ...
  console.log(`Found ${urls.length} Electrical activity images.`);
...
main().catch((error) => {
  console.error(
    "Unexpected failure while downloading Electrical activity images.",
  );
  console.error(error);
  process.exit(1);
});
