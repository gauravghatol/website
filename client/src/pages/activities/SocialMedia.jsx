    ...
    <GenericPage title="Social Media Team">
      <div className="mx-auto max-w-5xl space-y-6 sm:space-y-8">
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          <h2 className="mt-3 text-[clamp(1.4rem,3.7vw,1.875rem)] font-bold text-gray-900">
            Connect with SSGMCE online
          </h2>
          <p className="mt-3 max-w-2xl text-[clamp(0.92rem,1.7vw,1rem)] leading-relaxed text-gray-600">
          ...
          {SOCIAL_LINKS.map((item) => {
            ...
            return (
              <a
                ...
                className="group rounded-3xl border border-gray-200 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg sm:p-6"
              >
                <div className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl text-white ${item.buttonClass}`}>
                  <Icon className="text-xl" />
                </div>
                <h3 className="mt-5 text-[clamp(1rem,2.2vw,1.25rem)] font-bold text-gray-900">{item.label}</h3>
