...
const IQACAccordion = ({ items, defaultOpen, renderHeader, renderContent, getKey }) => {
      ...
      {items.map((item) => {
        ...
        return (
          <div key={key} className="border border-gray-100 rounded-lg overflow-hidden">
            ...
            {isOpen && <div className="px-4 pb-3 pt-1">{renderContent(item)}</div>}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div>
        ...
        {['All', ...categories].map((cat) => (
          <button key={cat} onClick={() => setActiveCategory(cat)}
            className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
              activeCategory === cat ? 'bg-ssgmce-blue text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>
            {cat}
          </button>
        ))}
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {filtered.map((video) => (
          <div key={video.youtubeId} className="border border-gray-100 rounded-lg overflow-hidden group">
            <div className="relative aspect-video bg-gray-100">
              {playingId === video.youtubeId ? (
                ...
                <button onClick={() => setPlayingId(video.youtubeId)} className="absolute inset-0 w-full h-full cursor-pointer">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/30 transition-colors">
                    <div className="w-12 h-12 rounded-full bg-white/90 flex items-center justify-center">
                      <svg className="w-5 h-5 text-ssgmce-blue ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                    </div>
                  </div>
                </button>
              )}
            ...
            <div className="p-3">
              <span className="text-[10px] font-medium text-ssgmce-orange bg-ssgmce-orange/10 px-1.5 py-0.5 rounded">{video.category}</span>
              ...
              .map((section, index) => (
                ...
                >
                  ...
                  >
                        ...
                        {section.content.stats.map((stat, idx) => (
                          <React.Fragment key={idx}>
                            ...
                            <div className="text-center">
                              <span className={`block text-2xl font-bold ${idx % 2 === 0 ? 'text-ssgmce-blue' : 'text-ssgmce-orange'}`}>{stat.value}</span>
                    ...
                    {section.type === "meeting-records" && section.content.records && (
                      <IQACAccordion
                        items={section.content.records}
                        defaultOpen={section.content.records[0]?.year}
                        renderHeader={(item, isOpen) => (
                          <div className="flex items-center gap-3">
                            <span className={`text-sm font-semibold ${isOpen ? 'text-ssgmce-blue' : 'text-gray-700'}`}>{item.year}</span>
                            <span className="text-xs text-gray-400">{item.meetings.length} meeting{item.meetings.length !== 1 ? 's' : ''}</span>
                          </div>
                        )}
                        renderContent={(item) => (
                          <div className="grid sm:grid-cols-2 gap-2">
                            {item.meetings.map((m) => (
                              <a key={m.label} href={m.url} target="_blank" rel="noopener noreferrer"
                                ...
                                <span className="text-xs font-semibold text-ssgmce-blue bg-ssgmce-blue/5 px-2 py-0.5 rounded">{m.label}</span>
                                {m.date && <span className="text-xs text-gray-400">{m.date}</span>}
                                <span className="ml-auto text-xs text-gray-400 group-hover:text-ssgmce-blue transition-colors">PDF ↗</span>
                              </a>
                            ))}
                          </div>
                        )}
                    ...
                    {section.type === "year-reports" && section.content.reports && (
                      <IQACAccordion
                                ...
                                <div className="grid sm:grid-cols-2 gap-1.5">
                                  {item.criteria.map((c, i) => (
                                    <a key={i} href={c.url} target="_blank" rel="noopener noreferrer"
                                      ...
                                      <span className="font-semibold text-ssgmce-blue bg-ssgmce-blue/5 px-1.5 py-0.5 rounded">{c.num}</span>
                                      <span className="text-gray-600 group-hover:text-ssgmce-blue transition-colors">{c.title}</span>
                                    </a>
                                  ))}
                                </div>
                              </div>
                    ...
                    {section.type === "naac-criteria" && section.content.criteria && (
                      <IQACAccordion
                        items={section.content.criteria}
                        defaultOpen={section.content.criteria[0]?.num}
                        renderHeader={(item, isOpen) => (
                          <div className="flex items-center gap-3">
                            <span className="text-xs font-bold text-white bg-ssgmce-blue w-6 h-6 rounded flex items-center justify-center flex-shrink-0">{item.num}</span>
                            <span className={`text-sm font-semibold ${isOpen ? 'text-ssgmce-blue' : 'text-gray-700'}`}>{item.title}</span>
                            <span className="text-xs text-gray-400">{item.indicators.length} indicators</span>
                          </div>
                        )}
                        renderContent={(item) => (
                          <div className="space-y-1.5">
                            {item.indicators.map((ind) => (
                              <div key={ind.id} className="flex items-start gap-3 p-2.5 rounded border border-gray-50 text-xs">
                                <span className="font-mono font-semibold text-ssgmce-blue bg-ssgmce-blue/5 px-1.5 py-0.5 rounded whitespace-nowrap">{ind.id}</span>
                                <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium flex-shrink-0 ${ind.type === 'QlM' ? 'bg-ssgmce-orange/10 text-ssgmce-orange' : 'bg-ssgmce-blue/10 text-ssgmce-blue'}`}>{ind.type}</span>
                    ...
                    {/* Video Gallery */}
                    {section.type === "video-gallery" && section.content.videos && (
                      <VideoGallery videos={section.content.videos} channelUrl={section.content.channelUrl} />
                    )}

                    {/* Document Grid */}
                    {section.type === "document-grid" && section.content.documents && (
                      <div className={`grid ${section.content.columns === 2 ? 'sm:grid-cols-2' : 'sm:grid-cols-3'} gap-2`}>
                        {section.content.documents.map((doc, idx) => (
                          <a key={idx} href={doc.url} target="_blank" rel="noopener noreferrer"
                            ...
                            {doc.year && <span className="text-xs font-semibold text-ssgmce-blue bg-ssgmce-blue/5 px-2 py-1 rounded">{doc.year}</span>}
                            <span className="text-gray-600 group-hover:text-ssgmce-blue transition-colors">{doc.label || 'View PDF ↗'}</span>
                            {!doc.label && <span className="ml-auto text-xs text-gray-400 group-hover:text-ssgmce-blue">↗</span>}
                          </a>
                        ))}
                      </div>
                    )}
                    ...
                    {/* Process Steps */}
                    {section.type === "process-steps" && section.content.steps && (
                      <div className="space-y-2">
                        {section.content.steps.map((step, i) => (
                          <div key={i} className="flex gap-3 items-start">
                            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-ssgmce-blue/5 text-ssgmce-blue text-[11px] font-bold flex items-center justify-center mt-0.5">{i + 1}</span>
                    ...
                    {section.type === "info-cards" && section.content.items && (
                      <div className={`grid ${section.content.columns === 1 ? 'grid-cols-1' : 'sm:grid-cols-2'} gap-3`}>
                        {section.content.items.map((item, idx) => (
                          <div key={idx} className="p-3 rounded-lg border border-gray-100">
                            <p className="text-sm font-semibold text-gray-800">{item.title}</p>
                            <p className="text-xs text-gray-500 mt-1">{item.description}</p>
                            {item.ownership && <p className="text-[10px] text-ssgmce-blue mt-2 font-medium">{item.ownership} · {item.timeFrame}</p>}
                          </div>
                        ))}
                      </div>
                    )}
